...
type
  TFrmImgBiochemFotos = class(TForm)
    PnlImg: TPanel;
    PnlBtnBar: TPanel;
    BtnSave: TButton;
    BtnCancel: TButton;
    ...
    NiceSetFrmImgBiochemFotos: TNiceSettings;
    AppEventsImgBiochemFotos: TApplicationEvents;
    ...
    procedure AppEventsImgBiochemFotosShortCut(var Msg: TWMKey; var Handled: Boolean);
  private
...
var
  FrmImgBiochemFotos: TFrmImgBiochemFotos;
...
procedure TFrmImgBiochemFotos.FormCreate(Sender: TObject);
begin
NiceSetFrmImgBiochemFotos.RegKey:= NiceSetRegKeyPath;
//грузим параметры формы
NiceSetFrmImgBiochemFotos.LoadSettings;
...
if DMFIB.DSetAnketa.FN('SEX').AsInteger = 1
  then //если это мужчина
      Self.Caption:= 'Изображение бланка биохимии эякулята (пациент: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')'
  else //если это женщина
      Self.Caption:= 'Изображение бланка биохимии эякулята (пациентка: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')';

TmpFExt:= DMFIB.DSetTmpBlobVisit.FN('TMP_SPERMBIOCHEMICFOTOS_EXT').AsString;

FocusFlag:= True;
end;
//----------------------------------------------------------------------------

procedure TFrmImgBiochemFotos.FormShow(Sender: TObject);
var TmpFPath: String;
begin
//форматируем и отображаем заголовок
LblName.Caption:= FullPatName;
LblCaption.Caption:= ' - скан результата биохимического исследования эякулята пациента';

with DMFIB, DSetTmpBlobVisit do
  begin
  //если поле не пустое, то заливаем картинку
  if not FN('TMP_SPERMBIOCHEMICFOTOS').IsNull then
    begin
      TmpFPath:= FPath + '\SpermBioTmpImg' + FN('TMP_SPERMBIOCHEMICFOTOS_EXT').AsString;
      TBlobField(FN('TMP_SPERMBIOCHEMICFOTOS')).SaveToFile(TmpFPath);
        try
...
procedure TFrmImgBiochemFotos.BtnSaveClick(Sender: TObject);
begin
with DMFIB, DSetTmpBlobVisit do
  begin
    Edit;
    if ImgCont.Picture.Graphic <> nil
      then
        begin
          ImgCont.Picture.SaveToFile(FPath + '\SpermBioTmpImg.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_SPERMBIOCHEMICFOTOS')).LoadFromFile(FPath + '\SpermBioTmpImg.tmp');
          FN('TMP_SPERMBIOCHEMICFOTOS_EXT').AsString:= TmpFExt;
        end {if ImgCont.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_SPERMBIOCHEMICFOTOS')).Clear;
          FN('TMP_SPERMBIOCHEMICFOTOS_EXT').AsString:= '';
        end;
...
procedure TFrmImgBiochemFotos.AppEventsImgBiochemFotosShortCut(var Msg: TWMKey; var Handled: Boolean);
begin
if FocusFlag then
  begin
...
AppEventsImgBiochemFotos.CancelDispatch;//чтобы другие формы не реагировали по этому событию
end;

//----------------------------------------------------------------------------

procedure TFrmImgBiochemFotos.BtnCancelClick(Sender: TObject);
begin
...
procedure TFrmImgBiochemFotos.FormResize(Sender: TObject);
begin
...
procedure TFrmImgBiochemFotos.ClearPictImgbox(Sender: TImage);
begin
...
procedure TFrmImgBiochemFotos.RepaintSpBtnDel(Sender: TObject);
begin
...
procedure TFrmImgBiochemFotos.LastFocus(var Mess: TMessage);
begin
...
procedure TFrmImgBiochemFotos.LoadPictToImgbox(Sender: TImage; var FExt: String);
begin
FExt:= '';
...
procedure TFrmImgBiochemFotos.SpBtnAddClick(Sender: TObject);
begin
...
procedure TFrmImgBiochemFotos.SpBtnDelClick(Sender: TObject);
begin
...
procedure TFrmImgBiochemFotos.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
//сохраняем параметры формы
NiceSetFrmImgBiochemFotos.SaveSettings;
end;
