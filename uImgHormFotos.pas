...
type
  TFrmImgHormFotos = class(TForm)
    ...
    NiceSetFrmImgHormFotos: TNiceSettings;
    AppEventsImgHormFotos: TApplicationEvents;
    ...
    procedure AppEventsImgHormFotosShortCut(var Msg: TWMKey; var Handled: Boolean);
  private
...
var
  FrmImgHormFotos: TFrmImgHormFotos;
...
procedure TFrmImgHormFotos.FormCreate(Sender: TObject);
begin
NiceSetFrmImgHormFotos.RegKey:= NiceSetRegKeyPath;
//грузим параметры формы
NiceSetFrmImgHormFotos.LoadSettings;
...
if DMFIB.DSetAnketa.FN('SEX').AsInteger = 1
  then //если это мужчина
      Self.Caption:= 'Изображение бланка исследования гормонов крови (пациент: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')'
  else //если это женщина
      Self.Caption:= 'Изображение бланка исследования гормонов крови (пациентка: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')';

TmpFExt:= DMFIB.DSetTmpBlobVisit.FN('TMP_HORMFOTOS_EXT').AsString;

FocusFlag:= True;
end;
//----------------------------------------------------------------------------

procedure TFrmImgHormFotos.FormShow(Sender: TObject);
var TmpFPath: String;
begin
//форматируем и отображаем заголовок
LblName.Caption:= FullPatName;
LblCaption.Caption:= ' - скан результата исследования гормонов крови пациента';

with DMFIB, DSetTmpBlobVisit do
  begin
  //если поле не пустое, то заливаем картинку
  if not FN('TMP_HORMFOTOS').IsNull then
    begin
      TmpFPath:= FPath + '\HormFotosTmpImg' + FN('TMP_HORMFOTOS_EXT').AsString;
      TBlobField(FN('TMP_HORMFOTOS')).SaveToFile(TmpFPath);
        try
...
procedure TFrmImgHormFotos.BtnSaveClick(Sender: TObject);
begin
with DMFIB, DSetTmpBlobVisit do
  begin
    Edit;
    if ImgCont.Picture.Graphic <> nil
      then
        begin
          ImgCont.Picture.SaveToFile(FPath + '\HormFotosTmpImg.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_HORMFOTOS')).LoadFromFile(FPath + '\HormFotosTmpImg.tmp');
          FN('TMP_HORMFOTOS_EXT').AsString:= TmpFExt;
        end {if ImgCont.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_HORMFOTOS')).Clear;
          FN('TMP_HORMFOTOS_EXT').AsString:= '';
        end;
...
procedure TFrmImgHormFotos.AppEventsImgHormFotosShortCut(var Msg: TWMKey; var Handled: Boolean);
begin
if FocusFlag then
  begin
    //реагируем на "хоткеи"
...
AppEventsImgHormFotos.CancelDispatch;
end;

//----------------------------------------------------------------------------

procedure TFrmImgHormFotos.BtnCancelClick(Sender: TObject);
begin
...
procedure TFrmImgHormFotos.FormResize(Sender: TObject);
begin
...
procedure TFrmImgHormFotos.ClearPictImgbox(Sender: TImage);
begin
...
procedure TFrmImgHormFotos.RepaintSpBtnDel(Sender: TObject);
begin
SpBtnDel.Enabled:= (ImgCont.Picture.Graphic <> nil);
end;
...
procedure TFrmImgHormFotos.LastFocus(var Mess: TMessage);
begin
...
procedure TFrmImgHormFotos.LoadPictToImgbox(Sender: TImage; var FExt: String);
begin
FExt:= '';
...
procedure TFrmImgHormFotos.SpBtnAddClick(Sender: TObject);
begin
...
procedure TFrmImgHormFotos.SpBtnDelClick(Sender: TObject);
begin
...
procedure TFrmImgHormFotos.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
//сохраняем параметры формы
NiceSetFrmImgHormFotos.SaveSettings;
end;
