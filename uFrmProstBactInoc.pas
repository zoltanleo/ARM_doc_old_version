...
Self.Caption:= 'Бак.посев сока простаты (пациент: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')';
...
if ChkBoxDrug_16.Checked then
  ChkBoxDrug_16.Checked:= not ((AnsiLowerCase(Trim(EdtExtra1.Text)) = 'название препарата') or
                                                                                     (Trim(EdtExtra1.Text) = ''));
if ChkBoxDrug_17.Checked then
  ChkBoxDrug_17.Checked:= not ((AnsiLowerCase(Trim(EdtExtra2.Text)) = 'название препарата') or
                                                                                     (Trim(EdtExtra2.Text) = ''));
  ...
  try
    ...
    TmpTxt:= 'Бак.посев сока простаты ('+ FormatDateTime('DD.MM.YY', DTPickProstBactInocDate.DateTime) + '): ';
    with TmpRE do
      begin
        Lines.Add(TmpTxt);
        SelStart:= 0;
    ...
    if ChkBoxPathogen1.Checked then
      begin
        ...
        + VarToStr(NumEdtProst1.Value) + ' КОЕ/мл.';
        ...
        if NumEdtProst1.Value > 3 then
          begin
            ...
            if (ChkBoxDrug_15.Checked) and (RdBtn_R1_15.Checked or RdBtn_I1_15.Checked or RdBtn_S1_15.Checked) then
              begin
                BegCurPos:= Length(TmpRE.Text);//запоминаем позицию начала строки
                TmpTxt:= '* р-р Октенисепт - '
                                          + GetSusceptStr(EndCurPos,Index,RdBtn_R1_15,RdBtn_I1_15,RdBtn_S1_15);
                BegCurPos:= BegCurPos + Length('* р-р Октенисепт - ');//начало выделения
                FmtTxtInTmpRichEdit(TmpRE,BegCurPos,EndCurPos,Index,TmpTxt);//добавляем форматированную строку
              end;
    ...
    if ChkBoxPathogen2.Checked then
      begin
        ...
        + VarToStr(NumEdtProst2.Value) + ' КОЕ/мл.';
  ...
  //==================================================================================
    ...
    TmpRE.Text:= TextProstBactInoc;//деформатируем имеющийся для сравнения

    //анализируем имеющийся текст бак.посева
    if Trim(TmpRE.Text) = ''
      then //если рич-эдит был пустой
        TextProstBactInoc:= TmpTxt //смело изменяем его через переменную TextProstBactInoc
      else//если что-то содержит
        if TextProstBactInoc <> TmpTxt //если новый текст отличается от старого
          then
            if Application.MessageBox('Ранее введенный результат исследования бактериального посева' +
                         ' сока простаты на микрофлору сейчас будет заменен на вновь сформированный' +
                          ' текст. Разрешить замену?', 'Изменение данных', MB_YESNO + MB_ICONINFORMATION) = IDYes
              then TextProstBactInoc:= TmpTxt;//заменяем, если ответ "Да"
...
AppEventsFrmProstBactInoc.CancelDispatch;//чтобы другие формы не реагировали по этому событию
end;
