...
type
  TFrmImgUZIRen = class(TForm)
    ...
    NiceSetFrmImgUZIRen: TNiceSettings;
    AppEventsImgUZIRen: TApplicationEvents;
    ...
    procedure AppEventsImgUZIRenShortCut(var Msg: TWMKey; var Handled: Boolean);
...
procedure TFrmImgUZIRen.FormCreate(Sender: TObject);
begin
NiceSetFrmImgUZIRen.RegKey:= NiceSetRegKeyPath;
//грузим параметры формы
NiceSetFrmImgUZIRen.LoadSettings;
...
if DMFIB.DSetAnketa.FN('SEX').AsInteger = 1
  then //если это мужчина
      Self.Caption:= 'Изображения УЗИ почек и мочевого пузыря (пациент: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')'
  else //если это женщина
      Self.Caption:= 'Изображения УЗИ почек и мочевого пузыря (пациентка: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')';

with DMFIB, DSetTmpBlobVisit do
  begin
    TmpFExt_01:= FN('TMP_UZIRENFOTOS_01_EXT').AsString;
    TmpFExt_02:= FN('TMP_UZIRENFOTOS_02_EXT').AsString;
    TmpFExt_03:= FN('TMP_UZIRENFOTOS_03_EXT').AsString;
    TmpFExt_04:= FN('TMP_UZIRENFOTOS_04_EXT').AsString;
  end;
...
procedure TFrmImgUZIRen.FormShow(Sender: TObject);
var TmpFPath: String;
begin
//форматируем и отображаем заголовок
LblName.Caption:= FullPatName;
LblCaption.Caption:= ' - изображения УЗИ почек пациента';

with DMFIB, DSetTmpBlobVisit do
  begin
  //если поле не пустое, то заливаем картинку
  if not FN('TMP_UZIRENFOTOS_01').IsNull then
    begin
      TmpFPath:= FPath + '\UZIRenTmpImg_01' + FN('TMP_UZIRENFOTOS_01_EXT').AsString;
      TBlobField(FN('TMP_UZIRENFOTOS_01')).SaveToFile(TmpFPath);
        try
  ...
  //если поле не пустое, то заливаем картинку
  if not FN('TMP_UZIRENFOTOS_02').IsNull then
    begin
      TmpFPath:= FPath + '\UZIRenTmpImg_02' + FN('TMP_UZIRENFOTOS_02_EXT').AsString;
      TBlobField(FN('TMP_UZIRENFOTOS_02')).SaveToFile(TmpFPath);
        try
  ...
  //если поле не пустое, то заливаем картинку
  if not FN('TMP_UZIRENFOTOS_03').IsNull then
    begin
      TmpFPath:= FPath + '\UZIRenTmpImg_03' + FN('TMP_UZIRENFOTOS_03_EXT').AsString;
      TBlobField(FN('TMP_UZIRENFOTOS_03')).SaveToFile(TmpFPath);
        try
  ...
  //если поле не пустое, то заливаем картинку
  if not FN('TMP_UZIRENFOTOS_04').IsNull then
    begin
       TmpFPath:= FPath + '\UZIRenTmpImg_04' + FN('TMP_UZIRENFOTOS_04_EXT').AsString;
       TBlobField(FN('TMP_UZIRENFOTOS_04')).SaveToFile(TmpFPath);
        try
...
procedure TFrmImgUZIRen.Img1DblClick(Sender: TObject);
begin
  ActShowFullImageExecute(Img1);
end;

procedure TFrmImgUZIRen.Img2DblClick(Sender: TObject);
begin
  ActShowFullImageExecute(Img2);
end;

procedure TFrmImgUZIRen.Img3DblClick(Sender: TObject);
begin
  ActShowFullImageExecute(Img3);
end;

procedure TFrmImgUZIRen.Img4DblClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.RepaintSpBtnDel(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.FormResize(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.BtnSaveClick(Sender: TObject);
begin
with DMFIB, DSetTmpBlobVisit do
  begin
    Edit;
    if Img1.Picture.Graphic <> nil
      then
        begin
          Img1.Picture.SaveToFile(FPath + '\UZIRenTmpImg_01.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_UZIRENFOTOS_01')).LoadFromFile(FPath + '\UZIRenTmpImg_01.tmp');
          FN('TMP_UZIRENFOTOS_01_EXT').AsString:= TmpFExt_01;
        end {if Img1.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_UZIRENFOTOS_01')).Clear;
          FN('TMP_UZIRENFOTOS_01_EXT').AsString:= '';
        end;

    if Img2.Picture.Graphic <> nil
      then
        begin
          Img2.Picture.SaveToFile(FPath + '\UZIRenTmpImg_02.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_UZIRENFOTOS_02')).LoadFromFile(FPath + '\UZIRenTmpImg_02.tmp');
          FN('TMP_UZIRENFOTOS_02_EXT').AsString:= TmpFExt_02;
        end {if Img2.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_UZIRENFOTOS_02')).Clear;
          FN('TMP_UZIRENFOTOS_02_EXT').AsString:= '';
        end;

    if Img3.Picture.Graphic <> nil
      then
        begin
          Img3.Picture.SaveToFile(FPath + '\UZIRenTmpImg_03.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_UZIRENFOTOS_03')).LoadFromFile(FPath + '\UZIRenTmpImg_03.tmp');
          FN('TMP_UZIRENFOTOS_03_EXT').AsString:= TmpFExt_03;
        end {if Img3.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_UZIRENFOTOS_03')).Clear;
          FN('TMP_UZIRENFOTOS_03_EXT').AsString:= '';
        end;

    if Img4.Picture.Graphic <> nil
      then
         begin
          Img4.Picture.SaveToFile(FPath + '\UZIRenTmpImg_04.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_UZIRENFOTOS_04')).LoadFromFile(FPath + '\UZIRenTmpImg_04.tmp');
          FN('TMP_UZIRENFOTOS_04_EXT').AsString:= TmpFExt_04;
        end {if Img4.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_UZIRENFOTOS_04')).Clear;
          FN('TMP_UZIRENFOTOS_04_EXT').AsString:= '';
        end;
...
procedure TFrmImgUZIRen.ActShowFullImageExecute(Sender: TObject);
...
procedure TFrmImgUZIRen.AppEventsImgUZIRenShortCut(var Msg: TWMKey; var Handled: Boolean);
begin
if FocusFlag then
  begin
...
AppEventsImgUZIRen.CancelDispatch;//чтобы другие формы не реагировали по этому событию
end;

//----------------------------------------------------------------------------

procedure TFrmImgUZIRen.BtnCancelClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.ClearPictImgbox(Sender: TImage);
begin
...
procedure TFrmImgUZIRen.LastFocus(var Mess: TMessage);
begin
...
procedure TFrmImgUZIRen.LoadPictToImgbox(Sender: TImage; var FExt: String);
begin
FExt:= '';
...
procedure TFrmImgUZIRen.SpBtnUpLeftAddClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.SpBtnUpLeftDelClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.SpBtnUpRigthAddClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.SpBtnUpRigthDelClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.SpBtnDnLeftAddClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.SpBtnDnLeftDelClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.SpBtnDnRigthAddClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.SpBtnDnRigthDelClick(Sender: TObject);
begin
...
procedure TFrmImgUZIRen.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
//сохраняем параметры формы
NiceSetFrmImgUZIRen.SaveSettings;
end;
