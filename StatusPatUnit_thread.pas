...
uses
  ...
  NiceSettings, pFIBDataSet, AppEvnts;

type
  TMyThrd = class(TThread)
  private
    { Private declarations }
    FThrdStrValue: string;
  protected
...
uses MainAndrUnit, TblAnketaUnit, TblClinicUnit, TblDoctorUnit, TblPriceUnit,
     ...
     uFrmMicUrBactInoc, uFrmProstBactInoc, uFrmUrinBactInoc, uRichEdtor, uSplash;
...
begin
CanClose:= CloseFrmPermition;//передаем разрешение на закрытие формы из процедур

//если явно не разрешено закрытие окна без запроса, то спросим ;-)
if not CloseFrmPermition then
  begin
    if Application.MessageBox('Вновь введенные Вами данные или исправления будут отменены. Вы уверены, что хотите '
      ...
      then
        begin
          ...
          CanClose:= not CloseFrmPermition;
        end;
  end;
end;
...
begin
CloseFrmPermition:= False;//запретим закрываться окну без спроса
Close;
end;
...
with DMFIB do
  begin
    ...
    MyThrd:= TMyThrd.Create(True);

    MessageForm.Show('Начинаем сохранение данных ');
    Sleep(500);

    MessageForm.Text('Начинаем сохранение данных .');
    Sleep(500);
...
//    FrmSplash:= TFrmSplash.Create(nil);
    try
      ...
      MyThrd.Priority:= tpHigher;
      MyThrd.Resume;

      Application.ProcessMessages;

      MessageForm.Text(MesSplashText);
...
//        FrmSplash.Show;
        ...
        CloseFrmPermition:= True;//разрешим закрыть окно без запроса
        Close;//отправляемся в процедуру OnCloseQuery формы
        ...
        Application.MessageBox(PChar(TmpStr), 'Ошибка работы с базой данных', MB_ICONERROR + MB_OK);
        ...
        CloseFrmPermition:= False; //запретим закрывать окно без запроса
        Close;//теперь попытаемся закрыть окно, раз ничего не получилось
...
begin
  ...
  //переключаемся на русскую раскладку клавиатуры
  ...
  with DMFIB, TmpBlobQry do
    begin
      ...
      if VisitPatientFlag //если запись вставляется
        then
          begin
          ...
          begin
            SQL.Text:= 'INSERT INTO TMP_VISITBLOB ' +
...
//      ShowMessage('текущий ID = ' + IntToStr(DSetTmpBlobVisit.FN('TMP_ID_VISITBLOB').Value));
    end;
...
if VisitPatientFlag
  then
   AnamListNo:= 0  //если запись создается
    ...
    AnamListNo:= 2; //если запись редактируется, то мы можем не попасть на вкладку с жалобами,
...
procedure TMyThrd.DoWork;
begin
MesSplashText:= FThrdStrValue;
//FrmSplash.Label1.Caption:= FThrdStrValue;
end;

procedure TMyThrd.Execute;
begin
  inherited;
FThrdStrValue:= 'Дождитесь окончания процесса сохранения данных ...';
