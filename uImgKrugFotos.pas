...
type
  TFrmImgKrugFotos = class(TForm)
    ...
    NiceSetFrmImgKrugFotos: TNiceSettings;
    AppEventsImgKrugFotos: TApplicationEvents;
    ...
    procedure AppEventsImgKrugFotosShortCut(var Msg: TWMKey; var Handled: Boolean);
  private
...
var
  FrmImgKrugFotos: TFrmImgKrugFotos;
...
procedure TFrmImgKrugFotos.FormCreate(Sender: TObject);
begin
NiceSetFrmImgKrugFotos.RegKey:= NiceSetRegKeyPath;
//грузим параметры формы
NiceSetFrmImgKrugFotos.LoadSettings;
...
if DMFIB.DSetAnketa.FN('SEX').AsInteger = 1
  then //если это мужчина
      Self.Caption:= 'Изображение бланка теста Крюгера (пациент: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')'
  else //если это женщина
      Self.Caption:= 'Изображение бланка теста Крюгера (пациентка: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')';

TmpFExt:= DMFIB.DSetTmpBlobVisit.FN('TMP_KRUGFOTOS_EXT').AsString;

FocusFlag:= True;
end;
//----------------------------------------------------------------------------

procedure TFrmImgKrugFotos.FormShow(Sender: TObject);
var TmpFPath: String;
begin
//форматируем и отображаем заголовок
LblName.Caption:= FullPatName;
LblCaption.Caption:= ' - скан результата теста Крюгера пациента';

with DMFIB, DSetTmpBlobVisit do
  begin
  //если поле не пустое, то заливаем картинку
  if not FN('TMP_KRUGFOTOS').IsNull then
    begin
      TmpFPath:= FPath + '\KrugTmpImg' + FN('TMP_KRUGFOTOS_EXT').AsString;
      TBlobField(FN('TMP_KRUGFOTOS')).SaveToFile(TmpFPath);
        try
...
procedure TFrmImgKrugFotos.BtnSaveClick(Sender: TObject);
begin
with DMFIB, DSetTmpBlobVisit do
  begin
    Edit;
    if ImgCont.Picture.Graphic <> nil
      then
        begin
          ImgCont.Picture.SaveToFile(FPath + '\KrugTmpImg.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_KRUGFOTOS')).LoadFromFile(FPath + '\KrugTmpImg.tmp');
          FN('TMP_KRUGFOTOS_EXT').AsString:= TmpFExt;
        end {if ImgCont.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_KRUGFOTOS')).Clear;
          FN('TMP_KRUGFOTOS_EXT').AsString:= '';
        end;
...
procedure TFrmImgKrugFotos.AppEventsImgKrugFotosShortCut(var Msg: TWMKey; var Handled: Boolean);
begin
if FocusFlag then
  begin
...
AppEventsImgKrugFotos.CancelDispatch;//чтобы другие формы не реагировали по этому событию
end;

//----------------------------------------------------------------------------

procedure TFrmImgKrugFotos.BtnCancelClick(Sender: TObject);
begin
...
procedure TFrmImgKrugFotos.FormResize(Sender: TObject);
begin
...
procedure TFrmImgKrugFotos.ClearPictImgbox(Sender: TImage);
begin
...
procedure TFrmImgKrugFotos.RepaintSpBtnDel(Sender: TObject);
begin
...
procedure TFrmImgKrugFotos.LastFocus(var Mess: TMessage);
begin
...
procedure TFrmImgKrugFotos.LoadPictToImgbox(Sender: TImage; var FExt: String);
begin
FExt:= '';
...
procedure TFrmImgKrugFotos.SpBtnAddClick(Sender: TObject);
begin
...
procedure TFrmImgKrugFotos.SpBtnDelClick(Sender: TObject);
begin
...
procedure TFrmImgKrugFotos.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
//сохраняем параметры формы
NiceSetFrmImgKrugFotos.SaveSettings;
end;
