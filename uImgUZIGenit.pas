...
if DMFIB.DSetAnketa.FN('SEX').AsInteger = 1
  then //если это мужчина
      Self.Caption:= 'Изображения УЗИ гениталий (пациент: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')'
  else //если это женщина
      Self.Caption:= 'Изображения УЗИ гениталий (пациентка: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')';
...
LblCaption.Caption:= ' - изображения УЗИ гениталий';
...
with DMFIB, DSetTmpBlobVisit do
  begin
    ...
    if Img1.Picture.Graphic <> nil
      then
        begin
          Img1.Picture.SaveToFile(FPath + '\UZIGenTmpImg_01.tmp');//сохраняем во временный файл
    ...
    if Img2.Picture.Graphic <> nil
      then
        begin
          Img2.Picture.SaveToFile(FPath + '\UZIGenTmpImg_02.tmp');//сохраняем во временный файл
    ...
    if Img3.Picture.Graphic <> nil
      then
        begin
          Img3.Picture.SaveToFile(FPath + '\UZIGenTmpImg_03.tmp');//сохраняем во временный файл
    ...
    if Img4.Picture.Graphic <> nil
      then
         begin
          Img4.Picture.SaveToFile(FPath + '\UZIGenTmpImg_04.tmp');//сохраняем во временный файл
...
AppEventsImgUZIGenit.CancelDispatch;//чтобы другие формы не реагировали по этому событию
end;
