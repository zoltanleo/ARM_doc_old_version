...
uses
  ...
  FIBDataSet, NiceSettings, AppEvnts, ActnList, uShowFullImg, Menus, MainAndrUnit;

type
  TFrmImgStPraesens = class(TForm)
    PnlBtnBar: TPanel;
    PnlCapt: TPanel;
    PnlWork: TPanel;
    PnlUp: TPanel;
    ...
    NiceSetFrmImgStPraesens: TNiceSettings;
    AppEventsImgStPraesens: TApplicationEvents;
    SclBoxLeftUp: TScrollBox;
    Img1: TImage;
    SclBoxRightUp: TScrollBox;
    Img2: TImage;
    SclBoxLeftDown: TScrollBox;
    Img3: TImage;
    SclBoxRightDown: TScrollBox;
    Img4: TImage;
    ActList: TActionList;
    ...
    procedure AppEventsImgStPraesensShortCut(var Msg: TWMKey; var Handled: Boolean);
  ...
  public
    { Public declarations }
//    procedure ShowFullImage(Sender: TImage);
  end;

var
  FrmImgStPraesens: TFrmImgStPraesens;

implementation

uses DMFIBUnit, StatusPatUnit, VarAndrUnit, ScktMain;
...
procedure TFrmImgStPraesens.FormCreate(Sender: TObject);
begin
NiceSetFrmImgStPraesens.RegKey:= NiceSetRegKeyPath;
//грузим параметры формы
NiceSetFrmImgStPraesens.LoadSettings;
...
SclBoxRightDown.BorderStyle:= bsNone;
SclBoxRightUp.BorderStyle:= bsNone;
SclBoxLeftDown.BorderStyle:= bsNone;
SclBoxLeftUp.BorderStyle:= bsNone;

//рисуем заголовок в зависимости от пола
if DMFIB.DSetAnketa.FN('SEX').AsInteger = 1
  then //если это мужчина
      Self.Caption:= 'Изображения status praesens (пациент: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')'
  else //если это женщина
      Self.Caption:= 'Изображения status praesens (пациентка: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')';

with DMFIB, DSetTmpBlobVisit do
  begin
    TmpFExt_01:= FN('TMP_STPRESENSFOTOS_01_EXT').AsString;
    TmpFExt_02:= FN('TMP_STPRESENSFOTOS_02_EXT').AsString;
    TmpFExt_03:= FN('TMP_STPRESENSFOTOS_03_EXT').AsString;
    TmpFExt_04:= FN('TMP_STPRESENSFOTOS_04_EXT').AsString;
  end;
...
procedure TFrmImgStPraesens.FormShow(Sender: TObject);
var TmpFPath: String;
begin
//форматируем и отображаем заголовок
LblName.Caption:= FullPatName;
LblCaption.Caption:= ' - изображения общего статуса пациента';

with DMFIB, DSetTmpBlobVisit do
  begin
  if not FN('TMP_STPRESENSFOTOS_01').IsNull then
    begin
      TmpFPath:= FPath + '\StPrTmpImg_01' + FN('TMP_STPRESENSFOTOS_01_EXT').AsString;
      TBlobField(FN('TMP_STPRESENSFOTOS_01')).SaveToFile(TmpFPath);
        try
  ...
  if not FN('TMP_STPRESENSFOTOS_02').IsNull then
    begin
      TmpFPath:= FPath + '\StPrTmpImg_02' + FN('TMP_STPRESENSFOTOS_02_EXT').AsString;
      TBlobField(FN('TMP_STPRESENSFOTOS_02')).SaveToFile(TmpFPath);
        try
  ...
  //если поле не пустое, то заливаем картинку
  if not FN('TMP_STPRESENSFOTOS_03').IsNull then
    begin
      TmpFPath:= FPath + '\StPrTmpImg_03' + FN('TMP_STPRESENSFOTOS_03_EXT').AsString;
      TBlobField(FN('TMP_STPRESENSFOTOS_03')).SaveToFile(TmpFPath);
        try
  ...
  //если поле не пустое, то заливаем картинку
  if not FN('TMP_STPRESENSFOTOS_04').IsNull then
    begin
       TmpFPath:= FPath + '\StPrTmpImg_04' + FN('TMP_STPRESENSFOTOS_04_EXT').AsString;
       TBlobField(FN('TMP_STPRESENSFOTOS_04')).SaveToFile(TmpFPath);
        try
...
procedure TFrmImgStPraesens.Img1DblClick(Sender: TObject);
begin
  ActShowFullImageExecute(Img1);
end;

procedure TFrmImgStPraesens.Img2DblClick(Sender: TObject);
begin
  ActShowFullImageExecute(Img2);
end;

procedure TFrmImgStPraesens.Img3DblClick(Sender: TObject);
begin
  ActShowFullImageExecute(Img3);
end;

procedure TFrmImgStPraesens.Img4DblClick(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.RepaintSpBtnDel(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.FormResize(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.BtnSaveClick(Sender: TObject);
begin
with DMFIB, DSetTmpBlobVisit do
  begin
    Edit;
    if Img1.Picture.Graphic <> nil
      then
        begin
          Img1.Picture.SaveToFile(FPath + '\StPrTmpImg_01.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_STPRESENSFOTOS_01')).LoadFromFile(FPath + '\StPrTmpImg_01.tmp');
          FN('TMP_STPRESENSFOTOS_01_EXT').AsString:= TmpFExt_01;
        end {if Img1.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_STPRESENSFOTOS_01')).Clear;
          FN('TMP_STPRESENSFOTOS_01_EXT').AsString:= '';
        end;

    if Img2.Picture.Graphic <> nil
      then
        begin
          Img2.Picture.SaveToFile(FPath + '\StPrTmpImg_02.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_STPRESENSFOTOS_02')).LoadFromFile(FPath + '\StPrTmpImg_02.tmp');
          FN('TMP_STPRESENSFOTOS_02_EXT').AsString:= TmpFExt_02;
        end {if Img2.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_STPRESENSFOTOS_02')).Clear;
          FN('TMP_STPRESENSFOTOS_02_EXT').AsString:= '';
        end;

    if Img3.Picture.Graphic <> nil
      then
        begin
          Img3.Picture.SaveToFile(FPath + '\StPrTmpImg_03.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_STPRESENSFOTOS_03')).LoadFromFile(FPath + '\StPrTmpImg_03.tmp');
          FN('TMP_STPRESENSFOTOS_03_EXT').AsString:= TmpFExt_03;
        end {if Img3.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_STPRESENSFOTOS_03')).Clear;
          FN('TMP_STPRESENSFOTOS_03_EXT').AsString:= '';
        end;

    if Img4.Picture.Graphic <> nil
      then
         begin
          Img4.Picture.SaveToFile(FPath + '\StPrTmpImg_04.tmp');//сохраняем во временный файл
          TBlobField(FN('TMP_STPRESENSFOTOS_04')).LoadFromFile(FPath + '\StPrTmpImg_04.tmp');
          FN('TMP_STPRESENSFOTOS_04_EXT').AsString:= TmpFExt_04;
        end {if Img4.Picture.Graphic <> nil ..then}
      else //иначе канва пустая, поэтому "обнулим" поле
        begin
          TBlobField(FN('TMP_STPRESENSFOTOS_04')).Clear;
          FN('TMP_STPRESENSFOTOS_04_EXT').AsString:= '';
        end;
...
//----------------------------------------------------------------------------
//показываем увеличенную картинку в отдельном окне
procedure TFrmImgStPraesens.ActShowFullImageExecute(Sender: TObject);
...
procedure TFrmImgStPraesens.AppEventsImgStPraesensShortCut(var Msg: TWMKey; var Handled: Boolean);
begin
if FocusFlag then
  begin
    //реагируем на "хоткеи"
...
AppEventsImgStPraesens.CancelDispatch;
end;

//----------------------------------------------------------------------------

procedure TFrmImgStPraesens.BtnCancelClick(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.ClearPictImgbox(Sender: TImage);
begin
...
procedure TFrmImgStPraesens.LastFocus(var Mess: TMessage);
begin
...
procedure TFrmImgStPraesens.LoadPictToImgbox(Sender: TImage; var FExt: String);
begin
FExt:= '';
...
//      begin
//        Application.MessageBox('Размер загружаемого в БД файла не должен превышать 1 Mb!'
...
procedure TFrmImgStPraesens.SpBtnUpLeftAddClick(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.SpBtnUpLeftDelClick(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.SpBtnUpRigthAddClick(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.SpBtnUpRigthDelClick(Sender: TObject);
begin
...
//procedure TFrmImgStPraesens.ShowFullImage(Sender: TImage);
//var frmImage: TfrmShowFullImg;
//begin
//  if Assigned(@TImage(Sender)) then
//  begin
...
procedure TFrmImgStPraesens.SpBtnDnLeftAddClick(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.SpBtnDnLeftDelClick(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.SpBtnDnRigthAddClick(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.SpBtnDnRigthDelClick(Sender: TObject);
begin
...
procedure TFrmImgStPraesens.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
//сохраняем параметры формы
NiceSetFrmImgStPraesens.SaveSettings;
end;
