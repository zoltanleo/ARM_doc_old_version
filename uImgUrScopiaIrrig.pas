...
Self.Caption:= 'Изображения ирригационной уретроскопии (пациент: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')';
...
LblCaption.Caption:= ' - изображения ирригационной уретроскопии';
...
with DMFIB, DSetTmpBlobVisit do
  begin
    ...
    if Img1.Picture.Graphic <> nil
      then
        begin
          Img1.Picture.SaveToFile(FPath + '\UrScIrTmpImg_01.tmp');//сохраняем во временный файл
    ...
    if Img2.Picture.Graphic <> nil
      then
        begin
          Img2.Picture.SaveToFile(FPath + '\UrScIrTmpImg_02.tmp');//сохраняем во временный файл
    ...
    if Img3.Picture.Graphic <> nil
      then
        begin
          Img3.Picture.SaveToFile(FPath + '\UrScIrTmpImg_03.tmp');//сохраняем во временный файл
    ...
    if Img4.Picture.Graphic <> nil
      then
        begin
          Img4.Picture.SaveToFile(FPath + '\UrScIrTmpImg_04.tmp');//сохраняем во временный файл
    ...
    if Img5.Picture.Graphic <> nil
      then
        begin
          Img5.Picture.SaveToFile(FPath + '\UrScIrTmpImg_05.tmp');//сохраняем во временный файл
    ...
    if Img6.Picture.Graphic <> nil
      then
        begin
          Img6.Picture.SaveToFile(FPath + '\UrScIrTmpImg_06.tmp');//сохраняем во временный файл
...
AppEventsImgUrScopiaIrrig.CancelDispatch;//чтобы другие формы не реагировали по этому событию
end;
