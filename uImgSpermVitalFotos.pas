...
Self.Caption:= 'Изображение бланка жизнеспособности спермиев (пациент: ' + Trim(FrmMainProg.LblStatusSingle.Caption) + ')';
...
LblCaption.Caption:= ' - скан результата исследования жизнеспособности спермиев пациента';
...
with DMFIB, DSetTmpBlobVisit do
  begin
    ...
    if ImgCont.Picture.Graphic <> nil
      then
        begin
          ImgCont.Picture.SaveToFile(FPath + '\SpermVitalTmpImg.tmp');//сохраняем во временный файл
...
AppEventsImgSpermVitalFotos.CancelDispatch;//чтобы другие формы не реагировали по этому событию
end;
